 ...
 */
export async function generateTypes(options: { output?: string; } = {}) {
...
async function generateTypesFromFiles(files: string[]): Promise<string> {
  let output = `/**
...
`;
  ...
  const toolTypes = new Map<string, { input: string; output: string; }>();
...
function generateTypeFromZodSchema(zodContent: string): string {
...
function generateTypeFromObject(obj: unknown, indent = '  '): string {
